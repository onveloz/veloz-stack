  ...
  const [state, setState] = useQueryStates(
    {
      ...
      preset: parseAsStringEnum(enumVals(PresetId)).withDefault(
        DEFAULT_CONFIG.preset,
      ),
      /** Last named preset applied; kept when user customizes (`preset=custom`). */
      basePreset: parseAsStringEnum(enumVals(PresetId)).withDefault(
        DEFAULT_CONFIG.preset,
      ),
      frontend: parseAsStringEnum(enumVals(FrontendId)).withDefault(
        DEFAULT_CONFIG.frontend,
      ),
      mobile: parseAsStringEnum(enumVals(MobileId)).withDefault(
        DEFAULT_CONFIG.mobile,
      ),
      desktop: parseAsStringEnum(enumVals(DesktopId)).withDefault(
        DEFAULT_CONFIG.desktop,
      ),
      backend: parseAsStringEnum(enumVals(BackendId)).withDefault(
        DEFAULT_CONFIG.backend,
      ),
      runtime: parseAsStringEnum(enumVals(RuntimeId)).withDefault(
      ...
      dbHosting: parseAsStringEnum(enumVals(DbHostingId)).withDefault(
        DEFAULT_CONFIG.dbHosting,
      ),
      auth: parseAsStringEnum(enumVals(AuthId)).withDefault(
        DEFAULT_CONFIG.auth,
      ),
      deploy: parseAsStringEnum(enumVals(DeployId)).withDefault(
        DEFAULT_CONFIG.deploy,
      ),
      pm: parseAsStringEnum(enumVals(PackageManagerId)).withDefault(
        DEFAULT_CONFIG.pm,
      ),
      ui: parseAsStringEnum(enumVals(UiId)).withDefault(DEFAULT_CONFIG.ui),
      modules: parseAsArrayOf(
        parseAsStringEnum(enumVals({ options: MODULE_IDS })),
      ).withDefault(DEFAULT_CONFIG.modules),
      examples: parseAsArrayOf(
        parseAsStringEnum(enumVals(ExampleId)),
      ...
      lefthookAdvanced: parseAsBoolean.withDefault(
        DEFAULT_CONFIG.lefthookAdvanced,
      ),
